...
1. Em instalação nova, aplicar `supabase/migrations/0027_pix_comprovantes.sql` e depois `supabase/migrations/0028_comprovante_apenas_arquivo.sql`. Se a 0027 já foi aplicada, executar somente a 0028. Elas mantêm a tabela e o bucket privados e simplificam o envio para somente o arquivo.
...
- O cliente envia somente a imagem ou o PDF. Nome, valor, horário e autenticidade devem ser conferidos visualmente pela equipe; não há OCR, IA ou antivírus. PDFs são disponibilizados como download, não incorporados em HTML.
