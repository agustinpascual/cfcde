...
Os eventos recentes mostraram que a AxxonPay criou e devolveu o ID das cobranças, mas o navegador encerrou a etapa seguinte como falha de rede 3DS antes da autenticação terminar. As quatro ocorrências de 12/09 foram em iPhone e terminaram cerca de cinco segundos após a criação, padrão compatível com as repetições dos GETs CORS `checkout-config`/`get-checkout-info` do SDK Bloopi. Essas duas consultas são idempotentes e agora passam pela rota local `/api/pagamentos/bloopi-leitura`; `initiate-3ds`, desafio e `confirm-payment` continuam diretamente no navegador, sem repetição automática e preservando o IP usado na autorização. O proxy local do SDK também tentava primeiro `js.bloopi.io`, host sem DNS, antes de `app.bloopi.io`; a origem inválida foi removida do proxy e da CSP. O checkout registra uma fase fechada (`chamando_sdk`, `ambiente_aberto`, `desafio_aberto` ou `retorno_sdk`), sem mensagem crua nem dados pessoais/cartão, para distinguir indisponibilidade do provedor de cancelamento no desafio bancário.
