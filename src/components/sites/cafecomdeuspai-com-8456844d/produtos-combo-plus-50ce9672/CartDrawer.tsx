...
import { calcularDescontos, cupomValido } from "@/lib/promocoes";
...
export default function CartDrawer({open,quantity,onClose,onQuantityChange,productName=comboPlus.nome,productImage=comboPlus.imagem,unitPrice=comboPlus.ofertas[0].preco,productSlug=comboPlus.ofertas[0].slug}:Props){
 ...
 const descontos=calcularDescontos({subtotalCentavos:Math.round(subtotal*100),produtoSlug:productSlug,cupom:cupomAplicado});
 const total=subtotal-descontos.cupomCentavos/100;
 return <div className={`${styles.root} ${open?styles.open:""}`} aria-hidden={!open}>
  ...
  <aside className={styles.drawer} role="dialog" aria-modal="true" aria-labelledby="cart-title">
   ...
   <footer className={styles.footer}><div className={styles.summary}><span>Sub-total:</span><strong>{money.format(subtotal)}</strong></div>
    <div className={styles.coupon}><label htmlFor="cart-coupon">CUPOM DE DESCONTO</label><div><input id="cart-coupon" value={coupon} onChange={e=>{setCoupon(e.target.value);setCouponStatus("idle")}} placeholder="Digite seu Cupom"/><button type="button" onClick={apply}>Aplicar</button></div>{couponStatus!=="idle"&&<p className={couponStatus==="valid"?styles.success:styles.error} role="status">{couponStatus==="valid"?`Cupom ${cupomAplicado} aplicado!`:"Cupom inválido para este produto."}</p>}</div>
    {descontos.cupomAplicado&&<div className={styles.summary}><span>Cupom {descontos.cupomAplicado}:</span><strong>− {money.format(descontos.cupomCentavos/100)}</strong></div>}<div className={styles.total}><span>Total:</span><strong>{money.format(total)}</strong></div>{hasItem?<Link href={`/checkout?produto=${encodeURIComponent(productSlug)}${cupomAplicado?`&cupom=${encodeURIComponent(cupomAplicado)}`:""}`} className={styles.checkout}>Finalizar compra</Link>:<span className={`${styles.checkout} ${styles.disabled}`}>Finalizar compra</span>}<button className={styles.continue} type="button" onClick={onClose}>Continuar comprando</button>
   </footer>
  </aside>
 </div>
}
