    ...
    <div className={styles.store}>
      ...
      <main>
        <nav className={styles.breadcrumb} aria-label="Breadcrumb">
          <Link href="/">Início</Link><span>/</span><Link href="/vitrine">Produtos</Link><span>/</span><span>{product.name}</span>
        </nav>
        ...
        <section className={styles.product}>
          ...
          <div className={styles.info}>
            ...
            <p className={styles.installment}>{product.installment}</p>
        ...
        {recommendations.length ? <section className={styles.recommendations}><h2>Você também pode gostar</h2><div className={styles.grid}>{recommendations.map((item) => <Link href={`/produtos/${item.slug}/`} key={item.slug} className={styles.card}><div><Image src={item.image} alt={item.name} fill sizes="(max-width: 768px) 48vw, 280px" /></div><span>{item.category}</span><h3>{item.name}</h3>{item.originalPrice ? <del>{item.originalPrice}</del> : null}<strong>{item.price}</strong><small>{item.installment}</small></Link>)}</div></section> : null}
      </main>
      <SiteFooter />
      ...
      <CartDrawer open={cartOpen} quantity={Math.max(1, cartQuantity)} onClose={() => setCartOpen(false)} onQuantityChange={setCartQuantity} productName={product.name} productImage={product.image} unitPrice={product.priceCents / 100} />
    </div>
  );
}
